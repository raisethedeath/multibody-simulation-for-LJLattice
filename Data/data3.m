...
LJ1 = readmatrix('expansion/elastic bound,cell,ri=5,ro=20,v=1,t_tot=10,interval=0.1,dt=0.0002.csv');
% 设置视频写入对象
%v = VideoWriter('Dissipative,k=0.001,A=0.2,w=2,ri=5,ro=15,t=1,t_interval=0.05'); % 视频文件名
v= VideoWriter('Video/expansion/elastic bound,cell,ri=5,ro=20,v=1,t_tot=10,interval=0.1,dt=0.0002,new');
%v= VideoWriter('Video/relaxation/relaxation, cell,ri=5,ro=20,dt=0.0004,ti=2,v=1,t_tot=30,interval=0.1');
...
for t = 1:numFrames
    ...
    if (ri<=7)
        ri = ri0 + ve*(t-1)*t_interval;
        %title(['$r_i=$ ', num2str(ri),'$l_0$',', t=',num2str((t-1)*t_interval/t0),'$\tau_0$'],'interpreter', 'latex', 'FontSize', 10);
    end
    if (ri>7)
        ri=7;
