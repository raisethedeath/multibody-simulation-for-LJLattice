#ifndef SIMULATE_Triangular_DISSIPATIVE_H
#define SIMULATE_Triangular_DISSIPATIVE_H
...
void simulate_triangular_dissipative(std::vector<Particle>& particles, Energy& energy, float dt, const float e0, const float s0, float ro, float ri,float k0,float k) {
    ...
    for (size_t i = 0; i < numParticles; ++i) {
        ...
        for (size_t j = i + 1; j < numParticles; ++j) {
            ...
            if (particles[i].position.length() < ro && particles[j].position.length() < ro) {
                particles[i].applyForce(force-(f_d_i*(-1)) );
                particles[j].applyForce(-force-(f_d_j*(-1)) ); // 反向作用力
                energy.totalEp += Ep1(particles[i], particles[j], e0, s0);
            } else if (particles[i].position.length() < ro) {
                particles[i].applyForce(force-(f_d_i*(-1)));
                energy.totalEp += Ep1(particles[i], particles[j], e0, s0);
            } else if (particles[j].position.length() < ro) {
                particles[j].applyForce(-force-(f_d_j*(-1)));
                energy.totalEp += Ep1(particles[i], particles[j], e0, s0);
            }
